...
def _extract_mor_from_utterance(utt) -> Optional[str]:
    """
    Best-effort extraction of the CHILDES %mor tier for a single utterance.
    Falls back gracefully to None if not available in the object returned by pylangacq.
    """
    # 1) Common case: an attribute or tier dict on the utterance
    for attr in ("tiers", "_tiers", "dep_tiers"):
        tiers = getattr(utt, attr, None)
        if isinstance(tiers, dict):
            mor = tiers.get("%mor") or tiers.get("mor")
            if mor:
                if isinstance(mor, (list, tuple)):
                    return " ".join(str(x) for x in mor)
                return str(mor)

    # 2) Token-level morphology (very conservative; may not exist)
    tokens = getattr(utt, "tokens", None)
    if tokens:
        collected = []
        for t in tokens:
            # try common morphology-ish attributes
            for attr in ("mor", "morph", "morphemes", "pos"):
...
def iter_aligned_pairs(
    ...
    for det in details:
        ...
        for i in range(m):
            ...
            # %mor extraction (best-effort; may be None)
            mor = None
            try:
                mor = _extract_mor_from_utterance(utt)
            except Exception:
                mor = None
